...
- ANTI-TRONCATURE : le texte ne doit JAMAIS être coupé ou tronqué. Chaque bloc de texte doit être entièrement visible. Si un texte risque de déborder de son conteneur, raccourcis-le ou reformule-le pour qu'il tienne. Privilégie des phrases courtes et percutantes plutôt que de longs paragraphes qui seront coupés. Vérifie que chaque div/section a suffisamment d'espace pour son contenu : utilise word-wrap: break-word et overflow-wrap: break-word sur les conteneurs texte. N'utilise JAMAIS overflow: hidden sur un bloc contenant du texte narratif. Si nécessaire, réduis la taille de police (font-size) ou ajoute du padding pour que tout soit lisible sans troncature.
